...
- [ ] **Contained auth path**
  ...
  - Expected: Cursor Agent state is under `/Users/agent` paths; host Cursor IDE auth/profile state is unchanged and not copied into Hazmat's secret store.
  - Verify: `hazmat cursor-agent -- --version` round-trips, and a headless command works when you pass Cursor Agent's own `--print`/`--force`/`--trust` flags.

- [ ] **No host import path**
  - Preconditions: optional sentinel file under host `~/.cursor/` to prove host profile state is not imported.
  - Steps: `hazmat explain --for cursor-agent -C /tmp`; run `hazmat cursor-agent --no-backup -- --version`; inspect `/Users/agent` Cursor state paths and `hazmat config import --help`.
  - Expected: host sentinel files are unchanged; `hazmat config import` has no `cursor-agent` target; no Cursor host assets are synced.
...
- [ ] **Cloud credential storage**
  ...
  - Expected: output says no S3 objects are written until the first backup; `~/.hazmat/secrets/cloud/s3-access-key-id`, `s3-secret-key`, and `kopia-recovery-key` exist with mode `0600`; `~/.hazmat/config.yaml` contains endpoint/bucket but not the access key, secret key, recovery key, `access_key:`, `recovery_key:`, or `password:`. The bucket may still show `0 byte` until `hazmat backup --cloud -C <project>` succeeds for a selected project. If an old `~/.hazmat/cloud-credentials` file is present, the next cloud read migrates it into `~/.hazmat/secrets/cloud/s3-secret-key` and removes the legacy file.

- [ ] **Project-scoped cloud backup and restore**
  - Preconditions: disposable S3-compatible test credentials and a small disposable project directory.
  - Steps: run `hazmat backup --cloud -C <project>`; modify a file in `<project>`; run `hazmat restore --cloud -C <project>` with confirmation.
  - Expected: backup output names `<project>`; restore snapshots current `<project>` state before overwrite; restore affects only `<project>`, not sibling directories under the same parent.
...
- [ ] **Credential inventory and legacy residue**
  ...
  - Steps: run `hazmat doctor --fix` and approve the credential repair plan; optionally run `hazmat doctor --dry-run` first only when previewing the typed plan; run `hazmat check` and inspect the "Credential inventory" section. Use `hazmat migrate credentials --dry-run` only when validating the scoped lower-level migration command directly.
...
- [ ] **Docker Sandbox Git SSH fail-closed boundary**
  - Preconditions: use the same project SSH key config.
  - Steps: preview or launch that project with `--docker=sandbox`.
  - Expected: Hazmat rejects the session with a message that Docker Sandbox lacks a container-side adapter for the host-side Git SSH transport broker. The message must not suggest copying keys into the sandbox or exposing `ssh-agent`.
