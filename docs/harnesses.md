...
- **Import from host path:** `hazmat config import claude` stores `~/.claude/.credentials.json` plus Claude account/onboarding state in `~/.hazmat/secrets/claude/`, and copies the user-level `commands/` and `skills/` portable basics plus your git identity into Hazmat's managed state. Doesn't import `settings.json`, hooks, MCP, or session history (those stay host-only).
