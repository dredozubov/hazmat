...
| **Antigravity** | agy (pinned) | `hazmat harness update antigravity` | Not currently supported (requires an isolated Keychain adapter) | `ANTIGRAVITY_API_KEY` or `GEMINI_API_KEY` via `hazmat config agent` | not supported in v1 |
...
| Antigravity Keychain OAuth | Adapter required | Disabled until Hazmat can provide a per-harness keychain or item-scoped broker. Use the `ANTIGRAVITY_API_KEY` / `GEMINI_API_KEY` env path instead |
...
- **Durable auth storage:** `~/.hazmat/secrets/claude/credentials.json` and `~/.hazmat/secrets/claude/state.json`. Hazmat materializes them to `/Users/agent/.claude/.credentials.json` and `/Users/agent/.claude.json` only while a Claude session is active. If Claude's migrated `/Users/agent/.claude/.config.json` state file already exists, Hazmat mirrors portable account/onboarding state there too for the active session and harvests it back on exit.
...
- **Import from host path:** `hazmat config import claude` stores `~/.claude/.credentials.json` plus Claude account/onboarding state in `~/.hazmat/secrets/claude/`, and copies the user-level `commands/` and `skills/` portable basics plus your git identity into Hazmat's managed state. Account/onboarding state is reconciled from host Claude state when available; commands and skills stay copy-once. Doesn't import `settings.json` wholesale, hooks, MCP, or session history (those stay host-only); Claude sessions bridge only narrow non-secret startup display settings (`theme`, `tui`) to avoid repeated style onboarding.
...
- **Durable auth storage:** Antigravity v1 has no curated import or file-backed OAuth sync. `agy` keeps its config and runtime state under `/Users/agent/.gemini/antigravity-cli`. Interactive OAuth is disabled because agy stores it in the macOS Keychain and Hazmat does not yet have a per-harness keychain or item-scoped broker.
...
- **Subscription / OAuth path:** not currently supported. Granting agy the shared agent login keychain would also expose other harness credentials because Seatbelt permissions apply to the database rather than an individual Keychain item.
