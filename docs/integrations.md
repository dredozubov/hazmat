...
| `docker` | `Dockerfile`, `Containerfile`, `compose.yml`, `compose.yaml`, `docker-compose.yml`, `docker-compose.yaml` | — (Docker only works in Tier 3 Docker Sandbox mode; native containment never grants daemon access) | `DOCKER_HOST`, `DOCKER_TLS_VERIFY` | `.docker/`, `*.dockerfile.swp` |
