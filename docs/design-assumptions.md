...
**Only explicitly implemented harnesses work.** Hazmat now has a harness boundary, but it is still a built-in registry, not a plugin system. To use Aider, Crush, Devin, Kilo, Kimi, Kiro, Mistral Vibe, Trae CLI, Pi, or another unimplemented tool, you'd still need code changes for install/config/import behavior and, where necessary, seatbelt-visible state paths.
