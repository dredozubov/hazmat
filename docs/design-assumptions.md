...
**Concurrent sessions are possible but racy.** Each `hazmat claude`, `hazmat opencode`, `hazmat qwen`, `hazmat cursor-agent`, `hazmat shell`, or `hazmat exec` session gets a unique seatbelt policy (PID-based filename). But all sessions share the same agent home and harness state. Two harness instances writing to shared state like `~/.claude/`, `~/.config/opencode/`, `~/.qwen/`, or Cursor Agent profile paths simultaneously is undefined behavior. We don't prevent it.
...
**Only explicitly implemented harnesses work.** Hazmat now has a harness boundary, but it is still a built-in registry, not a plugin system. To use Aider, CodeWhale/DeepSeek TUI, Crush, Devin, GitHub Copilot CLI, Kilo, Kimi, Kiro, Mistral Vibe, OpenClaw, Qoder, Trae CLI, Pi, or another unimplemented tool, you'd still need code changes for install/config/import behavior and, where necessary, seatbelt-visible state paths.
