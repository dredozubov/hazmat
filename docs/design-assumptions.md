...
**Privileged file operations on agent-controlled paths never follow symlinks.** Any root operation that touches a path under `/Users/agent` is racing the contained agent, which owns its home and can swap any path component for a symlink. A plain `sudo chown`/`sudo chmod` follows symlinks at every component, so a swapped final component (TOCTOU) or a pre-planted parent symlink could redirect the root operation onto an arbitrary target — a privilege-escalation primitive. Session-time agent directory-ownership repair therefore runs through the hidden `__repair-agent-dir` subcommand, which anchors at the agent home and opens each component below it with `O_NOFOLLOW` before `fchown`/`fchmod` on the resulting fd; a symlink at any level fails the open instead of redirecting the operation. The anchor's own intermediate components are root-owned system directories (and on macOS `/var`, `/tmp`, `/etc` are themselves symlinks), so resolution above the anchor is trusted while everything the agent can influence is opened no-follow.
...
**Concurrent sessions are possible but racy.** Each `hazmat claude`, `hazmat opencode`, `hazmat qwen`, `hazmat cursor-agent`, `hazmat pi`, `hazmat shell`, or `hazmat exec` session gets a unique seatbelt policy (PID-based filename). But all sessions share the same agent home and harness state. Two harness instances writing to shared state like `~/.claude/`, `~/.config/opencode/`, `~/.qwen/`, `~/.pi/agent`, or Cursor Agent profile paths simultaneously is undefined behavior. We don't prevent it.
...
**Mixed state today.** API keys configured through `hazmat config agent`, GitHub API tokens configured through `hazmat config github`, file-backed harness auth imported or harvested from sessions, cloud backup secrets, typed provisioned Git SSH identities, and Git HTTPS credentials now live in `~/.hazmat/secrets/`. Codex and OpenCode auth also reconcile with only their registered host auth files by mtime; Hazmat does not sync whole harness homes. Materialized harness files are copied into `/Users/agent` only for matching sessions and harvested back out on normal exit. For Claude on keychain-preferring macOS releases, an OAuth refresh can rotate the live token into the agent login keychain while rewriting `~/.claude/.credentials.json` to a logged-out empty object; harvest and crash recovery therefore read the declared `Claude Code-credentials` item from the agent login keychain, promote it into `~/.hazmat/secrets`, and clear the agent keychain residue. Host login-Keychain reads/writes for that Claude item are opt-in via `HAZMAT_CLAUDE_HOST_KEYCHAIN_SYNC=1`; by default Hazmat does not touch the host login Keychain during startup, because macOS can require an interactive keychain-password unlock. GitHub API tokens are delivered only to sessions launched with `--github`. Git HTTPS uses a per-session brokered helper so git's built-in plaintext store is not durable agent-home state. Git SSH uses a session-scoped transport broker instead of exposing an ssh-agent socket. Antigravity's macOS Keychain OAuth is bridged by the agent login keychain adapter: agy (a flat native binary) gets the macOS Security framework trust-evaluation surface — the same one Codex uses — so its HTTPS/TLS and Google OAuth token exchange work in containment, and Hazmat prepares + unlocks the agent account login keychain (empty-password profile) and grants the session scoped read-write to only that keychain DB so agy stores its OAuth item there with no password prompt. This is a non-syncable external boundary: the agy Keychain item is never harvested into `~/.hazmat/secrets`, the host Keychain, or backups; it lives only in the agent login keychain. The write grant is scoped to the single agent login keychain DB + sidecars (the broad Keychains deny stays), identical to Claude's keychain scoping.
...
**Harnesses are explicit, and init no longer installs one by default.** `hazmat init` sets up containment first, then optionally bootstraps a supported harness such as Claude Code, Codex, OpenCode, Antigravity, Hermes, Qwen, Cursor Agent, or Pi when the operator selects one. The seatbelt still includes explicit support for Claude state in `~/.claude/`, but harness installation itself is now an opt-in step rather than an automatic side effect of init.
