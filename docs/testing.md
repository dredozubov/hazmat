...
path; fixture checks verify that local paths are readable directories or that
matching Hub cache snapshots exist under `HF_HUB_CACHE`, `HF_HOME`,
`XDG_CACHE_HOME`, or the default `$HOME/.cache/huggingface/hub` before the
network-disabled live smoke. PyTorch
