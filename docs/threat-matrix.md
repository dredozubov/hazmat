...
3. **SSH signing authority is intentionally narrow.** Hazmat does not pass the host user's `SSH_AUTH_SOCK` through integration env, and general SSH remains unsupported. Managed Git-over-SSH now uses a session-scoped brokered transport instead of a reusable session ssh-agent socket: the contained process can request Git upload/receive/archive transports for configured hosts, but it cannot open arbitrary SSH shells or sign unrelated SSH challenges through an exposed agent socket. Dedicated low-privilege project keys are still recommended.
...
- **No tier fully addresses child-process inheritance of explicitly granted credentials or MCP supply-chain attacks against credentials intentionally granted to the harness.** Hazmat blocks ambient credential env passthrough at the integration/session boundary and avoids exposing a reusable Git SSH agent socket, but downstream per-MCP scoping still requires harness or broker support.
