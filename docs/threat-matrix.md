...
| Agent writes `.hazmat/packs.yaml` to recommend packs for next session | N/A | N/A | Partial⁹ | Partial⁹ | Yes⁷ |
| Host-level attacker pre-fills `~/.hazmat/approvals.yaml` to bypass pack approval prompt | N/A | N/A | No¹⁰ | No¹⁰ | No¹⁰ |
...
9. **Agent can write `.hazmat/packs.yaml` but cannot auto-approve it.** The project directory is agent-writable, so the agent can create or modify the repo recommendations file. However, approval runs as the invoking user before the sandbox starts — the agent user cannot write to `~/.hazmat/approvals.yaml` (different UID, mode 0600). On the next session, the modified file triggers a re-prompt because the SHA-256 hash no longer matches the stored approval. The user sees exactly which packs are requested and must explicitly approve. Pack validation (credential deny check, safe env allowlist, no executable code) provides the hard security boundary regardless of approval state. See [stack-packs.md](stack-packs.md) for the full approval flow.

10. **Pack approval does not protect against host-level attackers.** If an attacker has code execution as the host user, they can pre-fill `~/.hazmat/approvals.yaml` to bypass the approval prompt. However, the blast radius is minimal: even with forged approvals, activated packs can only add read-only dirs (checked against the credential deny list), pass through env vars from the fixed safe set, and change backup excludes. Packs cannot widen write scope, expose credentials, execute code, or change network policy. An attacker with host-user access already has broader capabilities — they can directly edit `~/.hazmat/config.yaml`, install user packs in `~/.hazmat/packs/`, or tamper with the hazmat binary itself. The approval system protects against the sandboxed agent self-escalating, not against host compromise. Hardening approvals against host-level attackers would require hardware-backed trust (macOS Keychain or Secure Enclave), which is disproportionate to the pack system's deliberately limited blast radius. See [stack-packs.md](stack-packs.md) for the pack trust model and [design-assumptions.md](design-assumptions.md) for the approval trust boundary analysis.
...
- **Pack approval protects against agent self-escalation, not host compromise.** The approval prompt and hash-keyed record prevent the sandboxed agent from silently activating packs on the next session. They do not prevent an attacker with host-user code execution from forging approvals — but such an attacker already has broader access than packs can provide. Pack validation (not approval) is the security boundary.
