...
**SSH_AUTH_SOCK specifically:** Unset it in the agent environment. If a project needs git-over-ssh, use a Hazmat-managed per-project selection of a dedicated key from a chosen host-owned directory. The key stays in host-owned storage and the session receives only a brokered Git transport command, not the main user's agent socket or a reusable session-local agent socket.
