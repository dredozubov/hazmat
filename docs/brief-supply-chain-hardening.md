...
**SSH_AUTH_SOCK specifically:** Unset it in the agent environment. If a project needs git-over-ssh, use a deploy key in the agent user's `~/.ssh/` rather than forwarding the main user's agent socket.
