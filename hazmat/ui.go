...
func (u *UI) DoneBox(currentUser string) {
	...
	fmt.Println()
	cBold.Println("  1. Install Claude Code for the agent user:")
	fmt.Println("     hazmat bootstrap")
	fmt.Println()
	cBold.Println("  2. Set API key and git credentials:")
	fmt.Println("     hazmat enroll")
	fmt.Println()
	cBold.Println("  3. Verify and launch:")
	fmt.Println("     hazmat test")
	fmt.Println("     cd ~/workspace/my-project && hazmat claude")
	fmt.Println()
	cBold.Println("  4. Install LuLu for network monitoring (optional, recommended):")
	fmt.Println("     https://objective-see.org/products/lulu.html")
	fmt.Println()
	fmt.Println("  Check progress any time:  hazmat status")
	fmt.Println("  To uninstall:             hazmat rollback")
	fmt.Println()
}
