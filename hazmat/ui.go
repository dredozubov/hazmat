...
func (u *UI) Banner(currentUser string) {
	...
	fmt.Printf("    • Create a hidden '%s' user and '%s' group\n", agentUser, sharedGroup)
	fmt.Printf("    • Prepare %s with shared access\n", sharedWorkspace)
	fmt.Println("    • Install firewall rules (pf + DNS blocklist)")
	fmt.Println("    • Install Claude Code for the agent user")
	fmt.Println("    • Set your API key and git credentials")
	fmt.Println()
	fmt.Println("  After setup:  cd ~/workspace/my-project && hazmat claude")
	fmt.Println()
}
