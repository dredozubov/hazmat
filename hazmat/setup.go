...
# Installed by hazmat setup — do not edit manually.
#
# This wrapper is aliased to "claude" in the agent shell. It runs inside a
# session already confined by sandbox-exec (started via "hazmat shell" or
# "hazmat claude"), so no additional seatbelt policy is applied here.
set -euo pipefail
...
func runSetup(_ *cobra.Command, _ []string) (retErr error) {
	...
	if flagDryRun {
		...
		return fmt.Errorf("stdin is not a terminal\nFor non-interactive setup run: hazmat setup --yes")
...
// have run make install before 'hazmat setup'.  If the helper is absent, setup
...
func setupSudoers(ui *UI, r *Runner, currentUser string) error {
	...
	// Run 'hazmat test' after setup to exercise the full passwordless path.
	return nil
}
