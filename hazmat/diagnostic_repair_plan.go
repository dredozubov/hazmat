...
func diagnosticRepairNextStepsFor(plan diagnosticRepairPlan) []diagnosticRepairNextStep {
	...
	case "fix-yes", "fix-interactive":
		...
		steps = append(steps, diagnosticRepairNextStep{
			...
			Mutating:         false,
			RequiresApproval: true,
			Reason:           "Verify the host after approved repairs with launch-helper validation plus helper-backed, backup, and cloud live validation; ask before running in agent workflows.",
		})
	}
