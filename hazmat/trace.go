...
func supportedTraceHarnessSpecs() []debugtrace.HarnessSpec {
	return []debugtrace.HarnessSpec{
		...
		{
			ID:               debugtrace.HarnessID(HarnessAntigravity),
			DisplayName:      "Antigravity",
			CommandName:      "antigravity",
			LaunchCommand:    "hazmat antigravity",
			BootstrapCommand: "hazmat bootstrap antigravity",
			Installed:        installedTraceHarness(HarnessAntigravity),
			Explain:          traceExplain("antigravity", parseHarnessArgs),
			ProcessFilters: []string{
				"agy",
			},
			AgentStatePaths: []string{
				filepath.Join(agentHome, ".gemini", "antigravity-cli"),
			},
			HostStatePaths: []string{
				"~/.hazmat/secrets/providers/antigravity-api-key",
			},
			SampleArgs: []string{"-p", "say ok"},
		},
		{
