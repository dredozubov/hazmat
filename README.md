...
- **Docker Sandbox mode now covers every harness entrypoint.** `hazmat claude`, `hazmat codex`, `hazmat opencode`, `hazmat gemini`, `hazmat hermes`, `hazmat qwen`, `hazmat cursor-agent`, `hazmat shell`, and `hazmat exec` can all route into private-daemon Docker Sandbox sessions when the repo needs it.
